  ...
  return (
    <>
      {layerOrnaments.map((ornament) => (
        ...
        >
          <span
            className="pointer-events-none absolute top-0 left-1/2 overflow-hidden rounded-full bg-white/35 border-2 border-white/70 shadow-[0_12px_30px_rgba(0,0,0,0.25)] [width:calc(var(--size,44)*1px)] [height:calc(var(--size,44)*1px)] [transform:translateX(-50%)_translateX(calc(var(--x,0)*1px))_translateY(calc(var(--y,75)*1px))]"
            style={{
              ...cssVar("--x", ornament.x),
              ...cssVar("--y", ornament.y),
